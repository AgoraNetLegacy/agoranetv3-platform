...
-- Help & Support (owner-authorized 2026-08-22). This repository keeps one
-- consolidated PostgreSQL init migration; new model DDL is folded here and
-- schema-equivalence is enforced by tests/hardening.test.ts.
...
ALTER TABLE "SupportOperator" ADD CONSTRAINT "SupportOperator_profileId_fkey" FOREIGN KEY ("profileId") REFERENCES "Profile"("id") ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE "SupportCase" ADD CONSTRAINT "SupportCase_assignedOperatorId_fkey" FOREIGN KEY ("assignedOperatorId") REFERENCES "SupportOperator"("profileId") ON DELETE SET NULL ON UPDATE CASCADE;
ALTER TABLE "SupportInternalNote" ADD CONSTRAINT "SupportInternalNote_caseId_fkey" FOREIGN KEY ("caseId") REFERENCES "SupportCase"("id") ON DELETE CASCADE ON UPDATE CASCADE;
